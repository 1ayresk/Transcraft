...
public class FXSparkle extends EntityFX {
	...
	public static final ResourceLocation particles = new ResourceLocation(
			"transcraft", "textures/particles/particles.png");
	...
	public FXSparkle(World world, double d, double d1, double d2, float f,
			int type, int m) {
	...
	public FXSparkle(World world, double d, double d1, double d2, double x,
			double y, double z, float f, int type, int m) {
	...
	public void onUpdate() {
		...
		if (particleAge == 0 && tinkle && worldObj.rand.nextInt(10) == 0)
			worldObj.playSoundAtEntity(this, "random.orb", 0.02F,
					0.7F * ((worldObj.rand.nextFloat() - worldObj.rand
							.nextFloat()) * 0.6F + 2F));
		...
		if (leyLineEffect) {
			...
			FXSparkle fx = new FXSparkle(worldObj, prevPosX
					+ (worldObj.rand.nextFloat() - worldObj.rand.nextFloat())
					* .1f, prevPosY
					+ (worldObj.rand.nextFloat() - worldObj.rand.nextFloat())
					* .1f, prevPosZ
					+ (worldObj.rand.nextFloat() - worldObj.rand.nextFloat())
					* .1f, 1f, currentColor, 3 + worldObj.rand.nextInt(3));
			fx.noClip = true;
