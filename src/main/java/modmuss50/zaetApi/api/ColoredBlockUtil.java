...
public class ColoredBlockUtil {
	 ...
	 * 
	 * @param itemBlock
	 *            (ItemBlock) new ItemStack(coloredBlock).getItem(). Thank me
	 *            later :).
	 */
	 ...
	 * @param itemStack
	 * @param middle
	 * @param shouldUseMetaFromMiddle
	 *            Use meta for middle itemstack
	 */
	public static void addCraftingRecipeToItemStack(ItemStack itemStack,
			ItemStack middle, boolean shouldUseMetaFromMiddle) {
		for (int i = 15; i >= 0; i--) {
			if (shouldUseMetaFromMiddle) {
				GameRegistry.addShapelessRecipe(new ItemStack(itemStack.itemID,
						1, i), new ItemStack(middle.itemID, 1, i),
						new ItemStack(Item.dyePowder, 1, i), new ItemStack(
								Item.dyePowder, 1, i));
			} else {
				GameRegistry
						.addShapelessRecipe(
								new ItemStack(itemStack.itemID, 1, i),
								new ItemStack(middle.itemID, 1, middle
										.getItemDamage()), new ItemStack(
										Item.dyePowder, 1, i), new ItemStack(
										Item.dyePowder, 1, i));
			}
		}
	}
